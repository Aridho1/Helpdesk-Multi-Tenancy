    ...
    <div class="comment-list">
        <div class="comment-list-header">
            <h3>
                ...
                <span v-if="solutionComments.length" class="solution-count">• Solution {{ solutionComments.length }}</span>
            </h3>
        ...
        <div v-else>
            <div v-if="solutionComments.length > 0" class="comments solution-comments">
                <h4 class="section-title">Solution</h4>
                <CommentItem v-for="comment in solutionComments" :key="comment.id" :comment="comment" />
            </div>

            <div v-if="regularComments.length > 0" class="comments regular-comments">
                <h4 class="section-title">
                    {{ solutionComments.length ? "Other comments" : "Comments" }}
                </h4>
                <CommentItem v-for="comment in regularComments" :key="comment.id" :comment="comment" />
            </div>
        </div>

        <Dialog id="commentListModal" :visible="showModal" :header="t('tickets.detailPage.addComment')" :modal="true" :closable="true" :style="{ width: '600px' }" @update:visible="handleModalClose">
            <div class="comment-form">
                <div class="form-item">
                    <label for="commentListModalTextarea" class="form-label">Message</label>
                    <Textarea id="commentListModalTextarea" v-model="message" :placeholder="t('tickets.detailPage.writeComment')" :rows="4" class="form-input" />
                </div>
                <div class="form-item">
                    ...
                    <FileUpload id="commentListModalFileInput" v-model:files="files" :disabled="isPending" />
                </div>
            </div>

            <template #footer>
                <div class="modal-footer">
                    <Button id="commentListModalCancelBtn" :label="t('tickets.detailPage.cancel')" severity="secondary" @click="handleModalClose" />
                    <Button
