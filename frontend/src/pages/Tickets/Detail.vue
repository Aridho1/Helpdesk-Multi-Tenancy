...
const { data: ticket, isLoading: ticketLoading, error: ticketError } = useTicketDetail(ticketId.value);
...
const isSolutionLoading = computed(() => acceptMutation.isPending.value || rejectMutation.isPending.value);
...
const breadcrumbItems = computed(() => [{ label: t("tickets.list"), command: () => router.push("/tickets") }, { label: ticket.value?.pretty_id || `#${ticketId.value}` }]);
</script>

<template>
    <div class="ticket-detail-page">
        <Breadcrumb id="ticketDetailBreadcrumb" :model="breadcrumbItems" class="breadcrumb" />

        <div v-if="ticketLoading" class="loading-container">
            <ProgressSpinner id="ticketDetailLoading" style="width: 50px; height: 50px" strokeWidth="4" />
        </div>

        <div v-else-if="ticketError" class="error-container">
            <p>Error loading ticket: {{ ticketError.message }}</p>
            <Button id="ticketDetailBackBtn" :label="t('tickets.detailPage.backToList')" @click="router.push('/tickets')" />
        </div>

        <div v-else-if="ticket" class="ticket-content">
            <TicketActions :ticket="ticket" :is-update-pending="isUpdatePending" @update="openUpdateModal" @accept-solution="openAcceptSolution" @reject-solution="openRejectSolution" />
            ...
            <div class="comments-section">
                <CommentList :comments="comments || []" :ticket-id="ticketId" :loading="commentsLoading" />
            </div>
        </div>

        <SolutionModal :open="showSolutionModal" :mode="solutionMode" :loading="isSolutionLoading" @close="showSolutionModal = false" @submit="handleSolutionSubmit" />
